    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black text-slate-800 flex items-center gap-3">
          ...
          Gestion des Marchands & Applications
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {tenants.map((tenant) => {
          ...
          return (
            <div key={tenant.id} className="bg-white rounded-[2rem] border border-slate-200 p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center gap-4 flex-1">
                <div className="w-14 h-14 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 font-black text-xl">
                  {tenant.name[0]}
                </div>
                <div>
                  <h3 className="text-lg font-bold text-slate-800">{tenant.name}</h3>
                  <p className="text-sm text-slate-500 font-medium">{tenant.email} • /{tenant.slug}</p>
              ...
              <div className="flex items-center gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <select 
                    ...
                    className="p-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <button 
                    ...
                    className={`p-2 rounded-xl text-xs font-black uppercase transition-all ${
